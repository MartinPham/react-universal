...
export default {
	render(props) {
		// log('transition ', props.data.toJS())
		let origin = props.data.get('origin');
		if (origin) {
			origin = JSON.parse(origin);


			// window.revealInOrigin = origin;
			...
			if (typeof origin.top !== 'undefined') {
				...
				style.innerHTML = `.${transition}.pageTransition-enter> .Page { top:${origin.top}px; left:${origin.left}px; width:${origin.width}px; height:${origin.height}px; overflow: hidden; }`;
				style.innerHTML += `.${transition}.pageTransition-enter.pageTransition-enter-active> .Page { top:0px; left:0px; width:100%; height:100%; }`;
				style.innerHTML += `.${transition}.pageTransition-enter.pageTransition-enter-done> .Page { xoverflow: auto; }`;

				style.innerHTML += `.${transition}-back.pageTransition-exit> .Page { top:0px; left:0px; width:100%; height:100vh; }`;
				style.innerHTML += `.${transition}-back.pageTransition-exit.pageTransition-exit-active> .Page { top:${origin.top}px; left:${origin.left}px; width:${origin.width}px; height:${origin.height}px; overflow: hidden; }`;
				style.innerHTML += `.${transition}-back.pageTransition-exit.pageTransition-exit-active-done> .Page { xoverflow: auto; }`;
